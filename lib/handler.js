...
async function backup(config) {
	...
	const key = utils.generateBackupPath(config.PGDATABASE, config.ROOT);
...
async function handler(event) {
	const baseConfig = { ...DEFAULT_CONFIG, ...envConfig, ...event };
	const config =
		event.USE_IAM_AUTH === true ? decorateWithIamToken(baseConfig) : baseConfig;
	try {
